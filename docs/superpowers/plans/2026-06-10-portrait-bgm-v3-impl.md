...
| `rules/production/bgm.md` | Rewrite v2→v3 | 4 类选型 + per-topic 文档 + 评分卡 |
...
**Files:**
- Modify: `rules/production/bgm.md`（7 处改动：§0 新增 / §7 / §8.1 / §9 / §10 / §13 / §14 / §15 / 附录 B）
...
```bash
# 旧共享名应 0 命中
grep -c 'cyber_pulse\.mp3\|power_build\.mp3\|quiet_think\.mp3\|hop_pulse\.mp3' rules/production/bgm.md
# 预期: 0（除 §15 反模式中故意保留的"❌ 用 ... 共享预设名"那条）
# 新 per-topic 应 ≥ 3 命中
grep -c '<topic>\|per-topic' rules/production/bgm.md
# 预期: ≥ 5
```
...
```bash
git add rules/production/bgm.md
...
```bash
grep -nE 'power_build\.mp3|cyber_pulse\.mp3|quiet_think\.mp3|hop_pulse\.mp3' rules/production/bgm.md
...
- **bgm.md 改写错乱**：`git checkout HEAD~1 -- rules/production/bgm.md` 回滚单文件
