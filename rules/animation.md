...
## 14 · 横屏专用布局（16:9，1920×1080）

> **适用场景**：A 类人设/B 类健身知识在横屏格式下的布局。
> **与竖屏的本质区别**：横屏有更多横向空间，适合**左图右文**、**画中画**、**分栏对比**等布局。

### 14.1 横屏画布安全区（1920×1080）

| 区域 | 范围 | 说明 |
|---|---|---|
| **字幕安全区** | 底部 0-216px（20%）| 底部 80px 留白 |
| **人物头顶留白** | 顶部 0-108px（10%）| 头顶不贴边 |
| **左右安全区** | 两侧各 96px（5%）| 平台 UI 避让 |
| **内容核心区** | 中央 1728×864px（90%×80%）| 主要信息放这里 |
| **标题区** | 顶部 108-324px（10-30%）| 避免遮挡人脸 |
...
小窗位置：右下角，距边 96px，宽 540×303（29.7%×28%）
...
### 14.3 B 类横屏布局（健身知识/动作演示）

#### 14.3.1 动作演示主体

> 人物占中央 70%，参数 overlay 占右下角。
...
| 组件 | 状态 | 说明 |
|---|---|---|
| `SplitLeftRight` | 待建 | 左 40% 文 / 右 60% 图 |
| `PictureInPicture` | 待建 | 右下角小窗 overlay |
| `TripleSplit` | 待建 | 三分屏 |
| `OverlayCard` | 待建 | 右下角参数卡 |
...
| **安全区** | 四周各 5% | 顶部 7%，左右 5% |
| **分镜节奏** | 左右推进 | 上下滚动 |
| **转场** | 横向 Wipe/Push | 纵向 Slide |

### 14.7 Remotion 横屏组件模板

```tsx
// 左文右图（1920×1080）
<div style={{ position: "absolute", width: 1920, height: 1080 }}>
  {/* 左侧文字区 40% */}
  <div style={{ position: "absolute", left: 0, top: 0, width: 768, height: 1080 }}>
    <Title />
    <BulletPoints />
  </div>
  {/* 右侧人像 60% */}
  <div style={{ position: "absolute", left: 768, top: 0, width: 1152, height: 1080 }}>
    <OffthreadVideo src={speaker.mp4} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
  </div>
  {/* 底部字幕条 216px */}
  <div style={{ position: "absolute", left: 0, bottom: 0, width: 1920, height: 216 }}>
    <VoiceoverText />
  </div>
</div>
```
