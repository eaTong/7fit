...
| **图片素材占比** | 20% | 10% | 30% |
| **音频素材占比** | 20% | 10% | 30% |
| **核心素材** | 用户自拍口播 | 用户自拍动作演示 | 产品截图 + 操作录屏 |
| **mmx 生成** | 数据图 / B-roll | 数据图 / 抽象动效 | **80%**（UI 截图 / 数据图 / 动效）|
| **典型素材数** | 8-12 段 | 12-18 段 | 10-15 段 |

### 8.1 A 类素材清单骨架（v3 · 2026-06-10 双态重写）

> **A 类核心 = 主口播视频双用**。**1 段 ≥ 60s 主口播视频同时支撑口播态（全屏）和辅助素材态（右下角圆头像）**——不可拆、不可剪断。

```
视频素材（60%）
├── 001_talking_head.mp4  ← 【P0 必走】≥ 60s 主口播视频（贯穿全片，双用）
│   ├── 口播态 1：钩子（3s 切 1 段）
...
Step 2：录主口播视频 001_talking_head.mp4（≥ 60s，**按 5 段顺序录，不剪断**）
...
```
视频素材（40%）
├── 001_hook_screenshot.mov（功能截图录屏）
├── 010_broll_gym.mov（B-roll 健身房）
└── 020_action_broll.mov（B-roll 训练）

图片素材（30%）
├── 100_feature_1.png（功能 1 截图）
├── 101_feature_2.png（功能 2 截图）
└── 102_feature_3.png（功能 3 截图）
