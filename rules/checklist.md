...
>
> **6 大块 50+ 项检查**（A 文档/B 字幕/C 音频/D 素材/E 分镜/F 实现准备；含 G/H/I 扩展块共 56 项），输出 ✅ Ready to implement / render 或 ❌ Blocked + 修复建议。
>
...
>
> **2026-06-14 更新**：F.9 新增"动画清单已查"（实现前必须参考 [animation-inventory.md](animation-inventory.md)），总数 55→56 项。
...
开工 / 实现 / 渲染 前
  ↓
跑本文 6 大块 56 项检查（按 §7 场景化自检选对应大块）
  ↓
...
| **实现 Scene 前**（写代码）| A · 文档（7）+ B · 字幕（8）+ D · 素材（9）+ E · 分镜（10）+ F · 实现准备（9） | **43 项** |
| **准备渲染前**（最终门）| 全 6 大块 + G · BGM + H · 性能 + I · 平台 | **56 项** |
...
| **F.9** | **动画清单已查**（2026-06-14）| **实现前已参考** [animation-inventory.md](animation-inventory.md)，确认转场/动画/特效类型已从清单中选择 | [animation-inventory.md §1-3](animation-inventory.md#1-转场清单transition-types) |
