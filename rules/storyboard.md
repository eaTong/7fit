...
| 维度 | A 人设 | B 知识 | C 解码 |
|---|---|---|---|
| **总镜数** | 8-12 | **12-18** | 12-18 |
| **单镜时长** | 5-8s | 3-6s | 5-8s |
| **视频镜占比** | 60%（口播态 50% + 辅助素材态 50%）| **80%+** | 50%（含 code rendering）|
| **卡片镜占比** | 20% | 50% | 50% |
| **text_card 镜占比** | 20% | 10% | 15% |
| **B-roll 镜占比** | ≤ 20% | ≤ 30% | ≤ 15% |
| **转场风格** | 柔和（fade / slide_up）+ **双态切换 0.5s** | 紧凑（push_left / zoom）| 赛博解码（wipe/glitch/scanline）|
| **pause_breath 位置** | 主体中段 × 2-3 | **每动作后 × 4-6** | 段间 × 3-5 |
| **钩子段类型** | **talking_head**（人脸 + 大字幕）| **text_card**（大字钩子）| text_card（HUD/终端/代码）|
| **收尾段类型** | **talking_head**（人脸 + 评论图标）| text_card + 评论图标 | **text_card + 二维码 + CTA** |
...
```
钩子 (3s, text_card HUD/终端)
→ 造轮子 (8-12s, 代码素材/终端录屏)
→ 健脑 (8-12s, 功能截图/数据复盘)
→ 手术刀 (8-12s, 功能截图/操作视频)
→ pause_breath (0.7s)
→ 创始人视角 (5-8s, 口播/文字卡片)
→ 收尾 (7s, text_card + 二维码 + CTA)
```
